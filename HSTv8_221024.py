...
import string
from tkinter import NORMAL, IntVar, Tk, Label, StringVar, OptionMenu, Entry, Text, Scrollbar, RIGHT, Y, Listbox, YES, Button, Variable, \
...
master.title('High Severity Escalation App --Version 8.0')
...
with open(r'D:\Python Code\GitHub\HighSevTool\config.json') as json_file:
...
s1 = Entry(master, width=4)
s1.delete(0, END)
s2 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s3 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s4 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s5 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s6 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s7 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s8 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s9 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s10 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s11 = Entry(master, validate='key', validatecommand=vcmd, width=4)
s12 = Entry(master, validate='key', validatecommand=vcmd, width=4)


show_all = StringVar()


def showPercent(*args):
    service_degradation_variable = [s1.get(), s2.get(), s3.get(), s4.get(), s5.get(
    ), s6.get(), s7.get(), s8.get(), s9.get(), s10.get(), s11.get(), s12.get()]
    all_degradation = []
    print(str(service_degradation_variable[0]) + " 1st")

    try:
        for i in range(0, len(service_degradation_variable)):
            if not service_degradation_variable[0]:
                print(str(service_degradation_variable[i]) + " 2nd")
                print(str(all_degradation) + " N/A")
                show_all.set("N/A")
                ...
                print(str(all_degradation) + " append")
...
# text=(', '.join(op_items))
# show_lab = Label(master, text=(', '.join(show_all)))
show_lab = Label(master, textvariable=show_all)
show_lab.place(x=240, y=160)

show = Button(master, text="Show",
              command=showPercent)
show.place(x=280, y=120)
...
clicked = s1.bind('<Button-1>', click)
