from tkinter import CENTER, Tk, Label, StringVar, OptionMenu, Entry, Text, Scrollbar, RIGHT, Y, Listbox, YES, Button, \
    mainloop, END, Frame, messagebox, TclError, WORD, Menu, font, ttk, filedialog
...
master.title('High Severity Escalation App --Version 8.6')
...
def help():
    ...
    help_box.insert("end", "Help\n\n"
                           ...
                           "2. Bitly API can only do 100 free URLs a month."
                           " You can login to Bitly with our own account. Bitly API 每月只能做 100 個免費 UR."
                           " 您可以使用我們自己的帳戶登錄 Bitly.\n\n"
                           "3. Crisis Manager will default to Comms Manager if blank or 'Comms Manager' filled in. "
...
def refresh_ui_elements():
    ...
    action_taken_combobox.set("Choose a template...") # 重設提示文字

    # 2. 清空多行文字框，以防重新載入時內容重複
    action_taken_text_area.delete("1.0", END)

    # 3. 檢查 action_taken_options 列表是否為空
    if action_taken_options:
        # 如果列表不為空，就取第一個選項當作預設值
    ...
    # 確保這些元件變數是全域的，以便函式存取
    global status_options, severity_options, tier_options, root_cause_options, comms_manager_options
    ...
    if 'comms_manager_options' in globals() and comms_manager_options.winfo_exists(): comms_manager_options.destroy()

    # 2. 依序重建所有 OptionMenu
    # 重建 Status
    status_variable.set(status2[0] if status2 else "N/A")
    status_options = OptionMenu(master, status_variable, *(status2 if status2 else ["N/A"]))
    status_options.place(y=16, x=0)

    # 重建 Severity
    severity_variable.set(severity2[0] if severity2 else "N/A")
    severity_options = OptionMenu(master, severity_variable, *(severity2 if severity2 else ["N/A"]))
    severity_options.place(x=165, y=17)

    # 重建 Tier
    tier_variable.set(tier2[0] if tier2 else "N/A")
    tier_options = OptionMenu(master, tier_variable, *(tier2 if tier2 else ["N/A"]))
    tier_options.place(x=240, y=17)

    # 重建 Root Cause
    root_cause_variable.set(root_cause2[0] if root_cause2 else "N/A")
    root_cause_options = OptionMenu(master, root_cause_variable, *(root_cause2 if root_cause2 else ["N/A"]))
    root_cause_options.place(x=0, y=65)

    # 重建 Comms Manager
    comms_manager_variable.set(comms_manager2[0] if comms_manager2 else "N/A")
    comms_manager_options = OptionMenu(master, comms_manager_variable, *(comms_manager2 if comms_manager2 else ["N/A"]))
...
# 建立按鈕，可以將它放在您覺得適合的位置，例如 Print 按鈕旁邊
load_button = Button(master, text="Import\nConfig", command=open_file_and_reload)
load_button_font = font.Font(size=0, weight='bold')
load_button['font'] = load_button_font
load_button.config(height=2, width=10)
load_button.place(x=0, y=500) # 請自行調整位置
...
status_variable.set("---")  # default value
status_options = OptionMenu(master, status_variable, "---")
status_options.place(y=16, x=0)
...
severity_variable.set("---")  # default value
severity_options = OptionMenu(master, severity_variable, "---")
severity_options.place(x=165, y=17)
...
name = StringVar()
name_combobox = ttk.Combobox(master, textvariable=name, width=60, values=[])
name_combobox.place(x=0, y=115, height=25)
...
tier_variable.set("---")  # default value
tier_options = OptionMenu(master, tier_variable, "---")
tier_options.place(x=240, y=17)
...
s1.place(x=160, y=65)
s1.bind('<FocusIn>', select_all)
s2.place(x=190, y=65)
s2.bind('<FocusIn>', select_all)
s3.place(x=220, y=65)
s3.bind('<FocusIn>', select_all)
s4.place(x=250, y=65)
s4.bind('<FocusIn>', select_all)
s5.place(x=280, y=65)
s5.bind('<FocusIn>', select_all)
s6.place(x=310, y=65)
s6.bind('<FocusIn>', select_all)
s7.place(x=160, y=85)
s7.bind('<FocusIn>', select_all)
s8.place(x=190, y=85)
s8.bind('<FocusIn>', select_all)
s9.place(x=220, y=85)
s9.bind('<FocusIn>', select_all)
s10.place(x=250, y=85)
s10.bind('<FocusIn>', select_all)
s11.place(x=280, y=85)
s11.bind('<FocusIn>', select_all)
s12.place(x=310, y=85)
s12.bind('<FocusIn>', select_all)
...
# 1. 建立一個多行文字框 (Text Widget)
action_taken_text_area = Text(master, undo=True, wrap=WORD)
action_taken_text_area.place(x=400, y=50, width=390, height=100) # y往下移，為下拉選單留出空間
...
# 4. 建立下拉選單 (Combobox)
action_taken_combobox = ttk.Combobox(master, textvariable=action_taken_template_var, width=60, values=[])
action_taken_combobox.set("由此選擇 Action Taken 模板...")
action_taken_combobox.place(x=400, y=20, height=25)
# 5. 綁定事件，當使用者選擇一個項目時，就呼叫 on_template_select 函式
...
root_cause_variable.set("---")  # default value
root_cause_options = OptionMenu(master, root_cause_variable, "---")
root_cause_options.place(x=0, y=65)
...
comms_manager_variable.set("---")  # default value
comms_manager_options = OptionMenu(
    master, comms_manager_variable, "---")
comms_manager_options.place(x=400, y=170)
...
print_button.place(x=270, y=500)
...
countdown_button.place(x=135, y=500)
